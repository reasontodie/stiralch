...
#Админская очередь
@bot.message_handler(func=lambda message: message.text == 'Управление очередью' and message.chat.id in admins)
...
def start_command(message):
    if check_user_in_chat(chat_id, user_id):
        if message.chat.id in admins:
         ...
         bot.reply_to(message, "Извините, вы не можете пользоваться функционалом, так как вы не находитесь в чате 'Прання'.")
...
def queue_command(message):
    ...
    if not check_user_in_chat(chat_id, user_id):
        bot.reply_to(message, "Извините, вы не можете пользоваться функционалом, так как вы не находитесь в чате.")
        return
