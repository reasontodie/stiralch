...
def start_command_admin(message):
  if message.chat.id in admins:
    ...
    own_queue_btn = telebot.types.KeyboardButton('Управление очередью')
    ...
    bot.reply_to(message,
                 "Привет, папочка! Чем я могу помочь?",
                 reply_markup=markup)
  else:
    bot.reply_to(message, "Извините, вы не являетесь администратором.")

#Админская очередь
@bot.message_handler(func=lambda message: message.text == 'Управление очередью' and
                     message.chat.id in admins)
...
@bot.callback_query_handler(
  func=lambda call: call.data.startswith('delete_user'))
def delete_user_callback(call):
  ...
  if user_id in queue:
    ...
    bot.answer_callback_query(call.id,
                              text="Пользователь не найден в очереди.")
...
def start_command(message):
  if message.chat.id in admins:
    ...
    bot.reply_to(
      message,
      "Привет! Этот бот поможет тебе записаться на стирку и следить за очередью.",
      reply_markup=markup)

# Обработчик кнопки 'Записаться на стирку'
@bot.message_handler(
  func=lambda message: message.text == 'Записаться на стирку')
def queue_command(message):
  if message.chat.id in queue:
    bot.reply_to(message,
                 f"{message.from_user.first_name}, ты уже записан на стирку.")
  else:
    queue.append(message.chat.id)
    active_users[message.chat.id] = message.from_user.first_name
    bot.reply_to(
      message,
      f"{message.from_user.first_name}, ты успешно записался на стирку.")
    update_queue()
...
def done_command(message):
  if message.chat.id in active_users:
    ...
    bot.reply_to(
      message,
      f"{message.from_user.first_name}, ты уверен, что достирал и хочешь удалить свою запись?",
      reply_markup=markup)
    process_done_command_step(message)
  else:
    bot.reply_to(
      message,
      f"{message.from_user.first_name}, ты не записан на стирку или уже достирал."
    )
...
def process_done_command_step(message):
  if message.text == 'Да':
    if message.chat.id in active_users:
      ...
      bot.reply_to(
        message,
        f"{message.from_user.first_name}, спасибо, что позаботился об очереди."
      )
      update_queue()
      ...
      bot.reply_to(
        message,
        f"{message.from_user.first_name}, твоя запись на стирку не удалена.")

@bot.message_handler(
  func=lambda message: message.text == 'Отменить запись на стирку')
def cancel_command(message):
  if message.chat.id in queue:
    ...
    bot.reply_to(
      message,
      f"{message.from_user.first_name}, ты уверен, что хочешь отменить свою запись на стирку?",
      reply_markup=markup)
  else:
    bot.reply_to(message,
                 f"{message.from_user.first_name}, ты не записан на стирку.")

# Обработчик кнопок подтверждения отмены записи
@bot.message_handler(func=lambda message: message.text in ['Да', 'Нет'] and
                     message.chat.id in queue)
def cancel_confirm_command(message):
  if message.text == 'Да':
    ...
    bot.reply_to(
      message,
      f"{message.from_user.first_name}, твоя запись на стирку отменена.")
    update_queue()
    process_cancel_command_step(message)
  else:
    bot.reply_to(
      message,
      f"{message.from_user.first_name}, твоя запись на стирку не отменена.")
    process_cancel_command_step(message)
...
def process_error_report(message):
  ...
  bot.send_message(
    admins[0],
    f"Сообщение об ошибке от @{message.from_user.username}: {error_message}")
...
def view_queue_command(message):
  ...
  if not queue:
    ...
    return
  queue_text = "\n".join([
    f"{i+1}. @{bot.get_chat_member(chat_id, user_id).user.username}"
    for i, user_id in enumerate(queue)
  ])
...
def update_queue():
  if len(queue) > 0:
    queue_text = "\n".join([
      f"{i+1}. @{bot.get_chat_member(-1001703606698, user_id).user.username}"
      for i, user_id in enumerate(queue)
    ])
    ...
    bot.send_message(
      -1001703606698,
      f"В очереди никого нету, либо последний человек достирал :)")
